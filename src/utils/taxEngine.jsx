...
// 1. UPGRADE: LTCG Brackets mapped by Filing Status
const LTCG_BRACKETS = {
  'Single': [
...
export function calculateTaxes(incomeData, filingStatus = 'Single', stateLevel = 'Medium') {
  ...
  let passiveIncome = 0;
  let stcgIncome = 0; // Short Term Bucket
  let ltcgIncome = 0; // Long Term Bucket

  // 2. UPGRADE: Sort Income into the correct buckets
  incomeData.forEach(income => {
    if (!income.isTaxable) return;
    
    const amount = Number(income.gross) || 0;
    if (income.type === 'W-2 Salary' || income.type === 'W-2 Bonus') {
      w2Income += amount;
    } else if (income.type === 'Self-Employment (1099)') {
      selfEmployedIncome += amount;
    } else if (income.type === 'Short Term Capital Gains') {
      stcgIncome += amount;
    } else if (income.type === 'Long Term Capital Gains') {
      ltcgIncome += amount;
    } else {
  ...
  // Calculate FICA
  const w2Fica = w2Income * 0.0765; 
  const seFica = selfEmployedIncome * 0.153; 
  const totalFica = w2Fica + seFica;

  // 3. UPGRADE: Calculate Federal Income Tax (STCG goes in Ordinary Income!)
  ...
  // 4. UPGRADE: Calculate Long Term Capital Gains (The "Stacking" Math)
  const ltcgBracketsToUse = LTCG_BRACKETS[filingStatus] || LTCG_BRACKETS['Single'];
  let ltcgTax = 0;
  let ltcgRemaining = ltcgIncome;
  let incomeBaseForLtcg = taxableOrdinary; // LTCG stacks on top of ordinary income

  for (const bracket of ltcgBracketsToUse) {
    if (ltcgRemaining <= 0) break; // All LTCG taxed
    
    if (incomeBaseForLtcg < bracket.limit) {
      // Find out how much room is left in this specific LTCG bracket
  ...
  // 5. State Tax (States generally tax everything as ordinary income)
